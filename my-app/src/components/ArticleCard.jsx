...
const ArticleCard = ({ image, author, title, description, url, publishedAt }) => {
    ...
    const { toggleLike , likedCards} = useContext(likeContext)

    
    
    const navigate = useNavigate()
    
    const card = { image, author, title, description, url, publishedAt }
    const checkIfLiked = () => {

        return likedCards.some(item => item.url === card.url);
    ...
    return (
        <div className='shadow-2xl cursor-pointer hover:scale-102 duration-200 transition-all flex flex-col h-full'>
            <img className='w-full h-48 object-cover' src={image} alt="" />
            <div className='p-5 space-y-2 flex flex-col flex-grow'>
                <div onClick={handleCardClick} className='text flex flex-col hover:text-red-600 flex-grow '>
            ...
            <footer className='flex py-3 px-5 space-x-4 justify-center border-t-[1px]  border-gray-200 mt-auto'>
                <div onClick={() => { toggleLike({ image, author, title, description, url, publishedAt }) }} className='flex items-center space-x-1'>
                    <button className={`cursor-pointer hover:bg-red-200 p-1 rounded transition-colors ${checkIfLiked() && 'bg-red-200'}`}><svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className='flex items-center space-x-1'>
                    <span className='cursor-pointer'><svg width="13" height="16" viewBox="0 0 13 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className='flex items-center space-x-1'>
                    <span className='cursor-pointer'><svg width="10" height="16" viewBox="0 0 10 16" fill="none" xmlns="http://www.w3.org/2000/svg">
