...
const NewsGrid = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const response = await axios.get(`https://newsapi.org/v2/everything?q=apple&from=2025-10-02&to=2025-10-02&sortBy=popularity&apiKey=f273e871107a4ed7a9b7e03417a27c14`)
    ...
    const VisableArticles = data.slice(0, VisableCount)

    const hasMoreArticles = VisableCount < data.length
    ...
    return (
        <div className='max-w-6xl mx-auto px-4'>
            ...
            <div className='grid md:grid-cols-2 gap-5'>
                ...
                {
                    VisableArticles.map((item, index) => (
            ...
            </div>
            {
                hasMoreArticles && (
                    <div className='flex justify-center items-center my-6'>
                        <button onClick={loadMore} className='bg-transparent  text-red-500 py-3 px-6 font-medium cursor-pointer border border-red-500'>
                            View More
                        </button>
                    </div>
                )
